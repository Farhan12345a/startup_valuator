...
import time
import gradio as gr
from startup_agent_framework import JobAgentFramework
...
class App:
    ...
    def run(self):
        with gr.Blocks(title="Startup Valuation Is Right", fill_width=True) as ui:
            log_data = gr.State([])
            ...
            with gr.Row():
                gr.Markdown(
                    ...
                    "<strong>Startup Valuation Is Right</strong> - "
                    "Autonomous Agent Framework for valuation guessing opportunities"
                    "</div>"
                )
            with gr.Row():
                gr.Markdown(
                    ...
                    "collaborate to estimate startup valuations and surface "
                    "high-upside valuation gaps, then send "
                    "push notifications."
                    "</div>"
                )
            with gr.Row():
                opportunities_dataframe = gr.Dataframe(
                    headers=[
                        "Startup", "Sector", "Actual Valuation",
                        "AI Estimate", "Gap", "HQ/Region", "URL",
                    ],
                    wrap=True,
