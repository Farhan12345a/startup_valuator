...
class MessagingAgent(Agent):
    ...
    def alert(self, opportunity: JobOpportunity):
        text = f"Valuation Insight! Reported=${opportunity.listing.salary:,.0f}, "
        text += f"AI Estimate=${opportunity.estimate:,.0f}, "
        text += f"Gap=${opportunity.premium:,.0f}: "
        text += opportunity.listing.job_title[:30] + " (" + opportunity.listing.company[:20] + ")"
    ...
    def craft_message(
        ...
        user_prompt = (
            "Summarize this startup valuation opportunity in 2-3 exciting sentences for a push notification.\n"
            f"Startup Profile: {description}\n"
            f"Reported Valuation: ${offered_salary:,.0f}\n"
            f"Estimated Valuation: ${estimated_salary:,.0f}\n\n"
