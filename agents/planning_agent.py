...
class PlanningAgent(Agent):
    """
    Simple orchestrator: scan for startup profiles, estimate valuations,
    ...
    def run(self, listing: JobListing) -> JobOpportunity:
        self.log("Planning Agent is estimating valuation for a listing")
        estimate = self.ensemble.estimate(listing.description)
        premium = listing.salary - estimate
        self.log(f"Planning Agent processed listing - valuation gap ${premium:,.0f}")
    ...
    def plan(self, memory: List[str] = []) -> Optional[JobOpportunity]:
        """
        Full workflow:
        1. ScannerAgent finds startup profiles from RSS feeds
        2. EnsembleAgent estimates valuation for each
        3. MessagingAgent notifies user of best mispriced valuation opportunity
        """
        ...
        if selection:
            ...
            self.log(
                f"Planning Agent identified best opportunity with valuation gap ${best.premium:,.0f}"
            )
